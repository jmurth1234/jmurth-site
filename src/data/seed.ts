    ...
    if (page.frontmatter.type === 'project') {
      await payload.create({
        ...
        data: {
          ...
          showcase: page.frontmatter.showcase,
          showcaseOrder: page.frontmatter.showcaseOrder,
          shortDeck: page.frontmatter.shortDeck,
          role: page.frontmatter.role,
          timeframe: page.frontmatter.timeframe,
          archiveNote: page.frontmatter.archiveNote,
          techStack: page.frontmatter.techStack?.map((label: string) => ({ label })),
          highlights: page.frontmatter.highlights?.map((text: string) => ({ text })),
          outcomes: page.frontmatter.outcomes?.map((text: string) => ({ text })),
